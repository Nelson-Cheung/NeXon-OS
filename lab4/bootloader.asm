...
;常数，内核的起始逻辑扇区号 
core_start_sector equ 81 ;上限1000扇区，500K
    ...
    ;计算GDT所在的逻辑段地址
    ;GDT的32位物理地址，pdgt=0x0000_00007e00
    ...
    mov dword [ebx+0x18],0x7c00ffef    ;粒度为4KB 
...
setup:
    ...
    ;建立核心代码段描述符
    mov eax,0                 ;核心数据段起始汇编地址
    mov ebx,[edi]                 ;程序总长度
    dec ebx                            ;核心代码段界限
    add eax,edi                        ;核心代码段基地址
    mov ecx,0x00409200                 ;字节粒度的代码段描述符
    call make_gdt_descriptor
    mov [esi+0x30],eax
    mov [esi+0x34],edx
    
    mov word [0x7c00+pgdt],55          ;描述符表的界限
